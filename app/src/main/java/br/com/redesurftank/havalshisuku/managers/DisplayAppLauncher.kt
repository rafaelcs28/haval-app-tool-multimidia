...
object DisplayAppLauncher {
    ...
    private suspend fun recoverAndroidAutoClusterBlackByCyclingProjectionService(reason: String): Boolean {
        ...
        reportAndroidAutoBlackRecoveryToGitHub(reason, errBefore, errAfter, recovered)
        return true
    }

    // Sobe um relatório CURTO pro repo privado do usuário no GitHub (mesma via do botão Bug) sempre que
    // a black-recovery age, com o resultado — pra validar offline, sem telnet nem toque no botão.
    // Best-effort, fora da main. bugNumber=0 -> arquivo bug-0000-* (separa dos reports manuais, que
    // começam em #1).
    private fun reportAndroidAutoBlackRecoveryToGitHub(
        reason: String,
        errBefore: Int,
        errAfter: Int,
        recovered: Boolean
        ...
        Thread(
            {
                try {
                    ...
                    val logTail =
                        try {
                            sh(
                                "logcat -d -b main -t 400 2>/dev/null | grep -aiE 'VideoPlayer|androidauto|projection|Surface|MediaCodec' | tail -120 || true"
                            )
                        } catch (t: Throwable) {
                            "(logcat indisponivel: ${t.message})"
                        }
                    val body = buildString {
                        appendLine("# AUTO black-recovery do AA no cluster")
                        appendLine()
                        appendLine("- Resultado: **${if (recovered) "RECUPEROU" else "AINDA PRETO"}**")
                        appendLine("- Reason: `$reason`")
                        appendLine("- Erros do decoder (VideoPlayer) antes -> depois: **$errBefore -> $errAfter**")
                        appendLine("- Versao: ${BuildConfig.VERSION_NAME} (vc ${BuildConfig.VERSION_CODE})")
                        appendLine("- Stamp: $stamp")
                        appendLine()
                        appendLine("Detectado surface VALIDA + decoder em loop de IllegalState (valido-mas-preto);")
                        appendLine("ciclado o servico de projecao (com.ts.androidauto.projectionservice) pra re-criar o decoder.")
                        appendLine()
                        appendLine("## Logcat recente (VideoPlayer/projecao, pos-recuperacao)")
                        appendLine("```text")
                        append(logTail)
                        appendLine()
                        appendLine("```")
                    }
                    BugReportUploader.upload(ctx, body, 0, "blackrec-$stamp")
                } catch (t: Throwable) {
                    Log.e(TAG, "[$reason] auto-report da black-recovery falhou", t)
                }
            },
            "aa-blackrec-report"
        )
