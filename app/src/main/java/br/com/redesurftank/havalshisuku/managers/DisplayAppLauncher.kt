...
object DisplayAppLauncher {
    ...
    private fun getAndroidAutoDisplayBounds(displayId: Int): IntArray {
        val res = getDisplayResolution(displayId)
        // Cluster (display 3): desloca a janela do AA pra direita (OPT-IN). Comeca DESATIVADO;
        // so aplica se ENABLE_AA_CLUSTER_OFFSET estiver ligado. Borda direita fixa (res.first),
        // entao aumentar o offset so estreita a esquerda, nunca corta a direita.
        if (displayId == 3 &&
    ...
    // Re-aplica os bounds do AA no cluster ao vivo (quando o toggle/slider de offset muda),
    // pra ajustar sem reprojetar. Desligado -> getAndroidAutoDisplayBounds volta ao padrao.
